...
inquirer
  .prompt([
    ...
    },
    {
      ...
      validate: (input) => {
        ...
        return ['ui', 'components', 'includes', 'layouts'].includes(dirName)
      },
      transformer: (input) => {
  ...
  .then((answers) => {
    ...
    console.log(
      chalk.greenBright(
        '📜 Используем шаблон компонента с названием: ' +
          chalk.whiteBright(uppercamelcase(componentName))
      )
    )
    ...
    if (fs.existsSync(componentDirectory)) {
      console.error(
        chalk.red(
          `❌ Компонет ${chalk.whiteBright(uppercamelcase(componentName))} уже существует`
        )
      )
      // eslint-disable-next-line no-undef, no-process-exit
      process.exit(1)
    }
    ...
    console.log(
      `✅  ${chalk.greenBright('Создан компонент')} ${chalk.whiteBright(uppercamelcase(componentName))} ${chalk.greenBright('в директории')} ${chalk.whiteBright(dirName)}`
    )
  })
